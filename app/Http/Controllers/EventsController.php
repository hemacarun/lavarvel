...
class EventsController extends BaseController
{
    ...
    public function getEventsWithWorkshops() {
     //   throw new \Exception('implement in coding task 1');
        ...
        foreach($events as $record){
            $rows = $record;
            $id =  $record->id;
              $inner_result =    DB::table('workshops')->get()->where('event_id', $id); 
           
           

            $rows->workshops= $inner_result;
    ...
    public function getFutureEventsWithWorkshops() {
        ...
        $results = DB::select('select e.id, e.name, e.created_at, e.updated_at
             from events as e
             JOIN workshops as w ON w.event_id=e.id
               WHERE start> now() 
        ...
        foreach($results as $record){
            ...
            $inner_result =DB::select('select * from workshops
               WHERE start> now() and event_id =? order by id',[$record->id]);
